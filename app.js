...
function setupEventListeners() {
    ...
    // Le bouton import est desormais un <label> natif - l'ouverture du selecteur est geree par le HTML
...
function renameBlock(blockId, e) {
    ...
    renderBlockList();
    updateLivePreview();
    showToast(`Bloc renommé en Â«Â ${block.title}Â Â»`, 'success');
}
...
function updateLivePreview() {
    ...
    function isPageOverflowing(pageEl) {
        const contentWrapper = pageEl.querySelector('.doc-page-content-wrapper');
        const footer = pageEl.querySelector('.doc-page-footer');
        ...
        // Sécurité de 10mm en haut, 10mm en bas et 10mm d'écart minimum entre le contenu et le pied de page.
        // Marges totales requises = 30mm sur les 297mm de hauteur totale de page A4.
        const maxUsableRatio = (297 - 30) / 297;
        return (contentHeight + footerHeight) > (pageHeight * maxUsableRatio);
    }
...
function showProjectsModal() {
    ...
    if (sortedProjects.length === 0) {
        ...
        sortedProjects.forEach(proj => {
            ...
            const filledBlocks = proj.blocks ? proj.blocks.filter(b => b.content && b.content.trim()).length : 0;
...
function handleTranscriptionFileSelect(e) {
    ...
    if (fileType === 'txt') {
        ...
        showToast("Format de fichier non supporté (uniquement .docx, .txt, .pdf)", "error");
    }
    ...
    const newBlocks = [
        {
            id: generateBlockId(),
            title: 'OBJET DE LA VISITE',
            content: `Visite conseil réalisée le ${formattedDate} à l'adresse suivante : ${proj.visitAddress || 'Non renseignée'}.\n\nType de visite : ${visitTypeLabel}.\n\nCe rapport constitue un avis technique visuel fondé sur les observations réalisées lors de la visite. Il ne se substitue en aucun cas à une étude technique détaillée ou à un diagnostic structurel par un bureau d'études spécialisé.`
        }
    ];

    themes.forEach(theme => {
        const content = blocksContent[theme.key];
        if (content) {
            newBlocks.push({
                id: generateBlockId(),
                title: theme.title.toUpperCase(),
                content: content
            });
        }
    });

    // Ajouter une conclusion par défaut
    newBlocks.push({
        id: generateBlockId(),
        title: 'CONCLUSION GENERALE',
        content: `La visite a permis de passer en revue les principaux points de vigilance du bien. Il est recommandé de suivre les préconisations détaillées dans chaque section, notamment concernant la gestion de l'humidité et l'entretien de la toiture. Les désordres observés relèvent principalement du vieillissement naturel du bâti ancien et de techniques antérieures qui méritent d'être adaptées aux usages modernes (ventilation, isolation).`
    });
...
async function handleDownloadPdf() {
    ...
    try {
        ...
        // Swap relative src with base64 for PDF generation (bypasses local file CORS)
        const logoImgs = previewElement.querySelectorAll('.doc-logo-img');
        ...
        // Restore image src on error
        const previewEl = document.getElementById('document-preview-target');
        if (previewEl) {
            const logoImgs = previewEl.querySelectorAll('.doc-logo-img');
            logoImgs.forEach(img => {
